#This module offers the interfaces to perform the generation of the perfect hash table and its access interface
import sys
import mpht_code_printer
import dictionary
import ctypes
...
def PerfectHash( dictionary , loosePercentage) :
	...
	if len(dictionary)<100 and int(loosePercentage) < 100:
		loosePercentage = 100
	size = len(dictionary)
	Dsize = int((int(loosePercentage)*size)/100)
	folders = []
	...
	#We print out the generated code
	mpht_code_printer.printAll(disambiguationTable,table,Dsize,size)
